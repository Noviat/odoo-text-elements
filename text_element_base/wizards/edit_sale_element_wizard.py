...
class EditTextElementWizard(models.TransientModel):
    ...
    text_element_domain = fields.Char(compute="_compute_text_element_domain")
    text_element_custom_id = fields.Many2one(
        comodel_name="text.element.custom", string="Custom Text Element"
    )
    text_element_custom_domain = fields.Char(
    ...
    def _compute_text_element_domain(self):
        for wizard in self:
            if wizard.res_model:
                wizard.text_element_domain = json.dumps(
                    [
                ...
                wizard.text_element_domain = json.dumps([("res_model", "=", "all")])
    ...
    def _compute_text_element_custom_domain(self):
        for wizard in self:
            if wizard.res_model and wizard.res_id:
                wizard.text_element_custom_domain = json.dumps(
                    [
                ...
                wizard.text_element_custom_domain = json.dumps([("res_id", "=", 0)])
